...
"""
    문제 정의: Date를 계산하는 로직이 여러 클래스에 있어 일부만 다르고 여러 코드가 반복되는 문제가 있다.
...
class DateCalculator(ABC):
    ...
    def filter_expression(self):
        return {'profile': self.obj.id, 'profiledatestosnapshot__snapshot': None, 'profiledatestosnapshot__isnull': False}
...
class ClubsWithDateCalculator(DateCalculator):
    ...
    @ property
    def filter_expression(self):
        return {'club': self.obj.id, 'profiledatestosnapshot__snapshot': None, 'profiledatestosnapshot__isnull': False}
...
class ClubAvailableTimeSerializer(serializers.ModelSerializer):
    ...
    def get_intersection(self, obj):
        ...
        profile_dates = ProfileDates.objects.filter(
            club=obj.id, profiledatestosnapshot__snapshot=None, profiledatestosnapshot__isnull=False).select_related('date', 'profile')
